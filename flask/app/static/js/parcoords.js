...
(function (global, factory) {
    ...
    var uninstall$4 = function uninstall(state, pc) {
      return function () {
        pc.selection.select('svg').select('g.slider').remove();
        pc.on('axesreorder.strums', undefined);
    ...
    var selected$4 = function selected(brushGroup, state, config) {
      // Get the ids of the currently active strums.
      var sliders = Object.keys(state.sliderData).map(function (key) {
        return state.sliderData[key];
      }),
          brushed = config.data;
      ...
      return brushed.filter(function (d) {
        switch (brushGroup.predicate) {
          case 'AND':
            return sliders.every(function (slider) {
              return d[slider.axis] == slider.value;
            });
          case 'OR':
            return sliders.some(function (id) {
              return d[slider.axis] == slider.value;
            });
          default:
            throw new Error('Unknown brush predicate ' + config.brushPredicate);
    ...
    var onDragEnd$2 = function onDragEnd(brushGroup, state, config, pc, events) {
      return function () {
        var knob = state.sliderKnobs[state.active];
    ...
    var onDrag$2 = function onDrag(brushGroup, state, config, pc, events) {
      return function () {
        ...
        var newY = d3.scaleLinear().domain([dimensions[1], dimensions[0]]).range([0, height])(value);
    ...
    // logically only happen between two axes, so no movement outside these axes
    // should be allowed.
    var onDragStart$2 = function onDragStart(state, config, pc, axis, _selector) {
      return function () {
    ...
    var brushFor$2 = function brushFor(state, config, pc, events, brushGroup) {
      return function (axis, _selector) {
        var brushRangeMax = config.dimensions[axis].type === 'string' ? config.dimensions[axis].yscale.range()[config.dimensions[axis].yscale.range().length - 1] : config.dimensions[axis].yscale.range()[0];

        var slider = _selector.insert('rect').attr('class', 'slider-container').attr('height', brushRangeMax).attr('fill', '#00000000'); // If set to gone then drag doesn't work

        var _drag = drag();

        _drag.on('start', onDragStart$2(state, config, pc, axis, _selector)).on('drag', onDrag$2(brushGroup, state, config, pc, events)).on('end', onDragEnd$2(brushGroup, state, config, pc, events));
    ...
    var brushReset$4 = function brushReset(brushGroup, state, config, pc, events) {
      return function () {
        config.brushed = false;
        if (pc.g() !== undefined && pc.g() !== null) {
    ...
    var install$4 = function install(brushGroup, state, config, pc, events) {
      return function () {
        if (pc.g() === undefined || pc.g() === null) {
          pc.createAxes();
        }

        var slider = pc.g().append('svg:g').attr('class', 'slider').each(function (d) {
          brushFor$2(state, config, pc, events, brushGroup)(d, select(this));
        });

        slider.selectAll('rect').style('visibility', null).attr('x', -15).attr('width', 30);

        // Install the required brushReset function
        pc.brushReset = brushReset$4(brushGroup, state, config, pc, events);
      };
    };
    ...
    var install1DPoints = function install1DPoints(brushGroup, config, pc, events) {
      var state = Object.assign({}, BrushState$4);

      brushGroup.modes['1D-points'] = {
        install: install$4(brushGroup, state, config, pc, events),
        uninstall: uninstall$4(state, pc),
        selected: selected$4(brushGroup, state, config),
        brushState: function brushState() {
          return state.strums;
    ...
    var selected$5 = function selected(config) {
      var actives = [];
      var extents = [];
      var ranges = {};
    ...
    var brushReset$5 = function brushReset(config) {
      return function (dimension) {
        var brushesToKeep = [];
    ...
    var ParCoords = function ParCoords(userConfig) {
      ...
      pc.brushReset = brushReset$5(config);
      pc.selected = selected$5(config);
      ...
      install1DPoints(brush, config, pc, events);
